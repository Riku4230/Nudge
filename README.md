...
Hutch は現在、未署名・未公証の OSS 版として配布しています。安全性を重視する場合は、ソースコードを確認した上でローカルビルドしてください。
...
要件: macOS 14+ / Swift 5.9+ / Xcode Command Line Tools。

事前ビルド済みの `.dmg` / Homebrew Cask も提供していますが、現在は未署名のため上級者向けです。詳細は [インストール](#インストール) を参照。
...
Hutch は現在、Apple Developer ID による署名・公証済みビルドを提供していません。最も安全なインストール方法は、ソースコードを確認した上でローカル環境でビルドすることです。
...
このビルドは **未署名・未公証** です。macOS Gatekeeper により初回起動時に警告が表示される場合があります。安全性を重視する場合はソースからビルドしてください。

#### チェックサム検証

各 Release に `Hutch-vX.Y.Z.dmg.sha256` が添付されています。`.dmg` をダウンロードした場所と同じディレクトリで以下を実行：

```bash
shasum -a 256 -c Hutch-v0.2.0.dmg.sha256
```

`OK` と表示されればファイルが改竄されていません。

<details>
<summary>未署名ビルドを開く方法</summary>

macOS の警告を理解した上で続行する場合は、システム設定 → プライバシーとセキュリティ → 「このまま開く」から起動できます。Hutch を信頼できると判断した場合のみ行ってください。

</details>

### 3. 上級者向け: Homebrew Cask
...
Homebrew Cask 版も現在は **未署名・未公証** のビルドです。Cask 内で `.dmg` の SHA256 が固定されているため、Homebrew がダウンロード時に整合性を自動検証します。
...
階層表示には **フルディスクアクセス** の許可が必要：

```
システム設定 → プライバシーとセキュリティ → フルディスクアクセス → Hutch を ON
```

未許可でもサブタスクは追加できます（フラット表示にフォールバック）。
...
- LLM: [Anthropic Claude](https://www.anthropic.com/) / [OpenAI](https://openai.com/) / [Google Gemini](https://ai.google.dev/)
